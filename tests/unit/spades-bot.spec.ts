...
import { chooseBid, chooseCard, estimateBooks, type PlayContext } from '@/lib/spadesBot'
...
const ctx = (over: Partial<PlayContext> = {}): PlayContext => ({
  book: [], seat: 0, partnerSeat: 2, spadesBroken: true, myBid: 3, partnerBid: 3, ...over,
})
 ...
 * could follow, or taking a book while bidding nil.
 */
describe('chooseCard — legality', () => {
  it('never returns an illegal card when it must follow suit', () => {
    ...
    const c = chooseCard(hand, ctx({ book: [{ seat: 3, card: pip('hearts', 7) }] }))
  ...
  it('always returns a card that legalPlays agrees with', () => {
    ...
    for (const book of [[], [{ seat: 3, card: pip('clubs', 8) }], [{ seat: 3, card: pip('diamonds', 8) }]]) {
      const c = chooseCard(hand, ctx({ book }))
      const led = book.length ? effectiveSuit(book[0]!.card) : null
...
describe('chooseCard — tactics', () => {
  it('ducks rather than overtaking a partner who is already winning', () => {
    ...
    const c = chooseCard(hand, ctx({ book: [{ seat: 2, card: pip('hearts', 14) }] }))

    expect(cardId(c)).toBe('hearts-2')
  })

  it('wins as cheaply as it can rather than smashing the book', () => {
    const hand = [pip('hearts', 14), pip('hearts', 10), pip('hearts', 3)]
    const c = chooseCard(hand, ctx({ book: [{ seat: 3, card: pip('hearts', 9) }] }))
  ...
  it('ducks low when it cannot win the suit at all', () => {
    ...
    const c = chooseCard(hand, ctx({ book: [{ seat: 3, card: pip('hearts', 12) }] }))
  ...
  it('trumps with the cheapest winning spade when void', () => {
    ...
    const c = chooseCard(hand, ctx({ book: [{ seat: 3, card: pip('hearts', 14) }] }))
...
describe('chooseCard — nil', () => {
  it('stays under the book when bidding nil', () => {
    const hand = [pip('hearts', 13), pip('hearts', 2)]
    const c = chooseCard(hand, ctx({ myBid: 0, book: [{ seat: 3, card: pip('hearts', 9) }] }))

    // the king would win the book and break the nil
    expect(cardId(c)).toBe('hearts-2')
  })
  ...
  it('plays the highest losing card, not blindly the lowest', () => {
    ...
    const c = chooseCard(hand, ctx({ myBid: 0, book: [{ seat: 3, card: pip('hearts', 12) }] }))
  ...
  it('takes the book off a partner who bid nil rather than ducking', () => {
    const hand = [pip('hearts', 14), pip('hearts', 2)]
    const c = chooseCard(hand, ctx({
      partnerBid: 0, book: [{ seat: 2, card: pip('hearts', 9) }],
    }))
...
describe('chooseBid', () => {
  ...
  it('values trump length even without top spades', () => {
    ...
    expect(estimateBooks(longTrump)).toBeGreaterThan(estimateBooks(shortTrump))
  })
})
