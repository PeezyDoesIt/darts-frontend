<template>
  <div class="cricket">
      ...
      >
        ...
        <div class="pips-wrap">
          <span
            ...
            class="pip"
            :class="{
              existing: pipIsExisting(target, n),
              round:    pipIsRound(target, n),
            }"
          />
        </div>
    ...
    <div class="submit-row">
      ...
      <button class="submit-btn" @click="submit" :disabled="submitted">SUBMIT TURN</button>
    </div>
  </div>
</template>
...
function myClosed(target: CricketTarget) {
  return (existingMarks.value[target] ?? 0) >= 3
}

function pipIsExisting(target: CricketTarget, n: number) {
  return (existingMarks.value[target] ?? 0) >= n
}

function pipIsRound(target: CricketTarget, n: number) {
  const existing = existingMarks.value[target] ?? 0
  const hits = roundHits.value[target] ?? 0
  return existing < n && existing + hits >= n
}

// Tap tile: cycle roundHits 0 → max available → 0
...
.board-tile:not(:disabled):active {
  transform: scale(0.98);
}
.board-tile.active {
  border-color: var(--pink);
  background: rgba(255,45,120,0.18);
  box-shadow: 0 0 32px rgba(255,45,120,0.35), inset 0 0 40px rgba(255,45,120,0.08);
}
.board-tile.closed {
  opacity: 0.3;
  cursor: default;
...
.pip.existing {
  background: var(--pink);
  border-color: var(--pink);
  box-shadow: 0 0 20px rgba(255,45,120,1), 0 0 40px rgba(255,45,120,0.5);
}
.pip.round {
...
@media (max-width: 1024px) {
  .cricket { flex: 1; overflow: hidden; }
  ...
  .board-tile { padding: 0 12px; min-height: 44px; }

  .target-label { font-size: 26px; width: 60px; }

  .pips-wrap { gap: 12px; }

  .pip { width: 42px; height: 42px; }

  .pip-inner { width: 22px; height: 22px; }

  .hit-badge { font-size: 16px; width: 48px; }
  .closed-badge { width: 48px; font-size: 9px; }

  .submit-row {
    padding: 10px 12px;
    flex-shrink: 0;
  }
  .submit-btn { height: 52px; font-size: 18px; }
}
</style>
