  ...
  <div class="win-page" :style="{ background: `radial-gradient(ellipse at center, ${winner?.color}40 0%, #0a0a0a 70%)` }">
    ...
    <div class="win-inner">
      ...
      <div class="win-actions">
        <button class="btn btn-spray btn-xl" @click="playAgain">Play Again</button>
        <button class="btn btn-outline btn-lg" @click="goHome">Home</button>
      </div>
    </div>
  </div>
</template>
...
<style scoped>
.win-page { width: 100vw; height: 100vh; display: flex; align-items: center; justify-content: center; position: relative; overflow: hidden; }
.win-inner { display: flex; flex-direction: column; align-items: center; gap: 14px; position: relative; z-index: 1; }
...
.winner-avatar { width: 160px; height: 160px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 80px; overflow: hidden; border: 4px solid rgba(255,255,255,0.3); transition: box-shadow 0.3s; }
...
.final-scores { display: flex; flex-direction: column; gap: 8px; min-width: 340px; margin: 8px 0; }
.final-row {
  display: flex; align-items: center; gap: 12px; padding: 12px 16px;
  background: rgba(255,255,255,0.05); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);
  border-radius: 10px; border: 1px solid rgba(255,255,255,0.1); transition: all 0.15s;
}
...
.win-actions { display: flex; gap: 14px; margin-top: 8px; }
</style>
