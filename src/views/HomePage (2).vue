...
/* A child of the coin modal must never give up height: the modal scrolls, so there is no
   reason for one to shrink, and a squeezed child clips its own text. Pinned at every width,
   not just on a phone — the 2px Street border trimmed the content box and the narrow layout
   passed that loss straight to the header. */
.coin-modal > *, .settings-panel > * { flex-shrink: 0; }
