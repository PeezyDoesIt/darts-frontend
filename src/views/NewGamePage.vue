<template>
  <div class="ng-page">
    ...
    <div class="ng-header">
      ...
      <button class="btn btn-spray btn-lg" :disabled="selectedPlayers.length < 2" @click="startGame">
        START GAME →
      </button>
    </div>

    <div class="ng-body">
      <!-- LEFT: Game type + Timer -->
      <div class="ng-left">
        <section class="ng-section">
          <span class="label">Game Type</span>
          <div class="game-type-grid">
            <button
              v-for="(label, type) in GAME_TYPE_LABELS"
              :key="type"
              class="game-type-btn"
        ...
        <section class="ng-section">
          ...
          <div class="timer-options">
            <button
              v-for="t in timerOptions" :key="t"
              class="timer-btn" :class="{ active: timerDuration === t }"
              @click="timerDuration = t"
            >{{ t }}s</button>
          </div>
          <div class="custom-timer-row">
            <span class="hint">Custom:</span>
            <input v-model.number="timerDuration" type="number" min="10" max="300" class="input" style="width:90px" />
        ...
        <section class="ng-section">
          ...
          <div class="timer-options">
            <button
              v-for="t in throwTimerOptions" :key="t"
              class="timer-btn" :class="{ active: throwTimerDuration === t }"
              @click="throwTimerDuration = t"
            >{{ t === 0 ? 'Off' : t + 's' }}</button>
          </div>
          <div class="custom-timer-row">
            <span class="hint">Custom:</span>
            <input v-model.number="throwTimerDuration" type="number" min="0" max="300" class="input" style="width:90px" />
      ...
      <div class="ng-right">
        <section class="ng-section" style="flex:1;overflow:hidden;display:flex;flex-direction:column;gap:12px">
          <div class="players-header">
            ...
            <button class="btn btn-outline btn-sm" @click="router.push('/player-setup')">+ New Player</button>
          </div>

          <div v-if="playersStore.players.length === 0" class="empty-players">
            No players yet.
            <button class="link-btn" @click="router.push('/player-setup')">Add one →</button>
          </div>

          <div v-else class="player-grid scroll">
            <div
        ...
        <section v-if="selectedPlayers.length > 1" class="ng-section order-section">
          ...
          <div class="order-list">
            <div v-for="(p, i) in selectedPlayers" :key="p.id" class="order-row" :style="{ borderLeftColor: p.color, boxShadow: `inset 0 0 30px ${p.color}08` }">
              <span class="order-num display" :style="{ color: p.color }">{{ i + 1 }}</span>
              <div class="tile-avatar sm" :style="{ background: p.color, boxShadow: `0 0 10px ${p.color}80`, borderColor: 'rgba(255,255,255,0.2)' }">{{ p.avatarUrl ?? '🎯' }}</div>
              <span class="order-name">{{ p.name }}</span>
              <div class="order-btns">
                <button :disabled="i === 0" @click="moveUp(i)" class="btn btn-sm btn-surface">↑</button>
                <button :disabled="i === selectedPlayers.length - 1" @click="moveDown(i)" class="btn btn-sm btn-surface">↓</button>
              </div>
            </div>
          </div>
        </section>
...
<style scoped>
.ng-page { display: flex; flex-direction: column; width: 100vw; height: 100vh; overflow: hidden; }
...
.ng-body { flex: 1; display: flex; overflow: hidden; }
...
.game-type-btn.active { border-color: var(--pink); color: var(--pink); background: rgba(255,45,120,0.1); box-shadow: 0 0 12px rgba(255,45,120,0.2); }
...
.timer-btn.active { border-color: var(--blue); color: var(--blue); background: rgba(0,212,255,0.1); box-shadow: 0 0 12px rgba(0,212,255,0.2); }
...
.player-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 10px; overflow-y: auto; flex: 1; }

.player-tile {
  display: flex; align-items: center; gap: 14px; padding: 14px 16px;
  background: rgba(255,255,255,0.03); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);
  border: 2px solid rgba(255,255,255,0.06); border-radius: 4px;
  cursor: pointer; transition: all 0.2s; height: fit-content;
...
.tile-avatar {
  ...
  transition: box-shadow 0.2s; border: 2px solid rgba(255,255,255,0.15);
}
.tile-avatar.sm { width: 30px; height: 30px; font-size: 15px; border-radius: 3px; }
...
.order-row {
  ...
  border: 1px solid rgba(255,255,255,0.08); border-left: 3px solid rgba(255,255,255,0.1);
  border-radius: 4px; transition: all 0.15s;
}
.order-num { font-size: 28px; font-family: var(--font-display); width: 28px; text-align: center; letter-spacing: 0; }
...
@media (max-width: 1024px) {
  .ng-header { padding: 14px 20px; }
  .ng-body { flex-direction: column; overflow-y: auto; }
  .ng-left { width: 100%; border-right: none; border-bottom: 1px solid rgba(255,255,255,0.06); padding: 20px; gap: 20px; overflow-y: visible; }
  .ng-right { padding: 20px; overflow: visible; }
