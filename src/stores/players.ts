...
export const usePlayersStore = defineStore('players', () => {
  ...
  function playerToDb(p: Player, userId: string) {
    return {
      ...
      // Reuses the pip_theme column the migration already adds — same idea,
      // already in the schema, so this needs no new column.
      pip_theme: p.pipStyle,
  ...
  function dbToPlayer(row: Record<string, unknown>): Player {
    return {
      ...
      pipStyle: (row.pip_theme as import('../types/index').PipStyle | null) ?? null,
